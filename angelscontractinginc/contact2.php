...
/*********** Config ***************/
$companyName = "Angel’s Contracting Inc.";
$companyEmail = "info@angelscontractinginc.com"; //1,2,3... correos
$emailForThanks = "info@angelscontractinginc.com"; // 1 correo nada mas
...
if (!isset($_SESSION['token']) || $_SESSION['token'] != $_POST['token']) {
  $to = EMAILEOM;
    $subjectResp = "Blocked message in: $companyName";
    $txt = "This message has been blocked by Token";
    $headers = "From: no-reply@eliteonlinemedia.com";
  ...
  if (!empty($honeypot)) {
    //se enviar un correo notificando que se ha bloqueado un spam con el señuelo
    $to = EMAILEOM;
      $subjectResp = "Blocked message in: $companyName";
      $txt = "This message has been blocked by Honeypot";
      $headers = "From: no-reply@eliteonlinemedia.com";
    mail($to,$subjectResp,$txt,$headers);
      //inmediatamente redireccionamos a la URL donde se encuentra el contact form
      echo '<script language="javascript"> alert("Message sent successfully!!"); </script>';
    ...
    if(isset($_POST["captcha"])&&$_POST["captcha"]!=""&&$_SESSION["code"]==$_POST["captcha"]){    
      ...
      function validarDatos($email,$name,$subject,$message){
      ...
      $variablesLimpias = validarDatos($email,$name,$subject,$message);
      //variables limpias
